...
\urlstyle{same}
\hypersetup{
  pdftitle={Fenómenos Colectivos 2026-2},
...
\title{Fenómenos Colectivos 2026-2}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{index_files/mediabag/chapters/fluidos/../../images/fluidos/cilinder.pdf}}
...
{\centering \pandocbounded{\includegraphics[keepaspectratio]{chapters/fluidos/../../images/fluidos/tubo_u.png}}
...
\begin{tcolorbox}[enhanced jigsaw, colbacktitle=quarto-callout-tip-color!10!white, toptitle=1mm, titlerule=0mm, rightrule=.15mm, colframe=quarto-callout-tip-color-frame, leftrule=.75mm, breakable, colback=white, bottomrule=.15mm, coltitle=black, bottomtitle=1mm, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{Pista}, arc=.35mm, toprule=.15mm, left=2mm, opacitybacktitle=0.6, opacityback=0]
...
\chapter{Ondas mecánicas, ecuación de onda y
superposición}\label{ondas-mecuxe1nicas-ecuaciuxf3n-de-onda-y-superposiciuxf3n}
...
El estudio de las ondas mecánicas permite conectar una descripción
física intuitiva ---la propagación de una perturbación en un medio---
con una formulación matemática precisa basada en ecuaciones
diferenciales parciales. En una cuerda, en el sonido o en una
perturbación radial en un medio tridimensional, la información no se
transporta mediante el desplazamiento neto de materia a grandes
distancias, sino mediante la transmisión local de energía y momento a
través del medio.

Este capítulo desarrolla los conceptos centrales asociados con la
ecuación de onda, las ondas viajeras, las ondas esféricas y las ondas
estacionarias. El punto de partida son ejercicios típicos de un examen
parcial sobre ondas, donde se pide verificar soluciones, identificar
rapidez de propagación, localizar nodos y antinodos, y analizar la
superposición de ondas que viajan en sentidos opuestos. A partir de esas
ideas, construiremos una exposición más completa, con definiciones
formales, interpretación física y ejemplos computacionales en Python.

El objetivo no es únicamente resolver expresiones particulares, sino
mostrar cómo se reconocen patrones generales. Una función de la forma
\(f(x-vt)\) describe una perturbación que se desplaza hacia la derecha
con rapidez \(v\), mientras que una función \(f(x+vt)\) describe una
perturbación que se desplaza hacia la izquierda. Cuando dos ondas de
igual frecuencia, amplitud y longitud de onda viajan en sentidos
opuestos, su interferencia produce una onda estacionaria, caracterizada
por nodos fijos y antinodos de máxima oscilación (\textbf{feynman1964?};
\textbf{kleppner2014?}).

\section{La ecuación de onda
unidimensional}\label{la-ecuaciuxf3n-de-onda-unidimensional}

La ecuación de onda unidimensional modela la evolución de una
perturbación transversal \(y(x,t)\) que se propaga a lo largo de una
dirección espacial \(x\). En una cuerda ideal bajo tensión constante,
esta ecuación puede deducirse aplicando la segunda ley de Newton a un
elemento diferencial de cuerda y suponiendo pequeñas pendientes, de modo
que la tensión actúa aproximadamente con magnitud constante
(\textbf{marion2013?}; \textbf{french1971?}).

\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, rightrule=.15mm, colframe=quarto-callout-note-color-frame, leftrule=.75mm, breakable, arc=.35mm, toprule=.15mm, left=2mm, colback=white, opacityback=0]

Definición. La ecuación de onda unidimensional para una perturbación
\(y(x,t)\) con rapidez de propagación \(v\) es
\begin{equation}\phantomsection\label{eq-wave-1d}{
\frac{\partial^2 y}{\partial t^2}=v^2\frac{\partial^2 y}{\partial x^2}.
}\end{equation}

\end{tcolorbox}

Una característica fundamental de esta ecuación es que admite soluciones
viajeras de forma arbitraria. Si \(f\) es una función suficientemente
diferenciable, entonces \[
y(x,t)=f(x-vt)
\] es una onda que se desplaza hacia valores crecientes de \(x\),
mientras que \[
y(x,t)=g(x+vt)
\] es una onda que se desplaza hacia valores decrecientes de \(x\). La
solución general en una dimensión puede escribirse como la suma de ambas
contribuciones: \[
y(x,t)=f(x-vt)+g(x+vt),
\] resultado conocido como solución de d'Alembert
(\textbf{courant1962?}).

\subsection{Verificación directa}\label{verificaciuxf3n-directa}

Para verificar que \(y(x,t)=f(x-vt)\) satisface la ecuación de onda,
definimos \(u=x-vt\). Por regla de la cadena, \[
...
\section{Ondas esféricas}\label{ondas-esfuxe9ricas}

En tres dimensiones, una onda que se propaga radialmente desde una
fuente puntual no conserva amplitud constante. A medida que el frente de
onda se expande, la energía se distribuye sobre superficies esféricas de
área \(4\pi r^2\). Por ello, la amplitud de una onda esférica ideal
suele decaer como \(1/r\) en ausencia de absorción.

En problemas con simetría radial, la ecuación de onda tridimensional
puede reducirse a una ecuación efectiva para la cantidad \(r\xi(r,t)\).
Esto permite estudiar la propagación radial usando una ecuación
formalmente similar a la ecuación de onda unidimensional.

\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, rightrule=.15mm, colframe=quarto-callout-note-color-frame, leftrule=.75mm, breakable, arc=.35mm, toprule=.15mm, left=2mm, colback=white, opacityback=0]

Definición. Una onda esférica radial \(\xi(r,t)\) satisface la ecuación
reducida \begin{equation}\phantomsection\label{eq-spherical-wave}{
...
\] lo cual verifica que \(\xi=(1/r)f(r+vt)\) es solución de la ecuación
radial.

La presencia de \(r+vt\) indica que la forma de onda se desplaza hacia
valores decrecientes de \(r\), es decir, corresponde a una onda esférica
convergente. En cambio, una dependencia \(f(r-vt)/r\) representaría una
onda esférica saliente.

\section{Identificación de rapidez de
propagación}\label{identificaciuxf3n-de-rapidez-de-propagaciuxf3n}

Una onda sinusoidal viajera suele escribirse como \[
y(x,t)=A\sin(kx-\omega t+\phi),
\] donde \(A\) es la amplitud, \(k\) el número de onda, \(\omega\) la
frecuencia angular y \(\phi\) una fase inicial. La rapidez de fase se
obtiene imponiendo que la fase permanezca constante: \[
...
Definición. Para una onda armónica de fase \(kx-\omega t+\phi\), la
rapidez de fase es \begin{equation}\phantomsection\label{eq-wave-speed}{
v=\frac{\omega}{k}=\lambda f,
}\end{equation}

\end{tcolorbox}

donde \(\lambda=2\pi/k\) es la longitud de onda y \(f=\omega/(2\pi)\) es
la frecuencia ordinaria.

\subsection{Ejemplo: rapidez a partir de una expresión de
onda}\label{ejemplo-rapidez-a-partir-de-una-expresiuxf3n-de-onda}

Considérese una onda del tipo \[
y(x,t)=A\sin(kx-\omega t),
\] con \[
A=3.50\,\text{mm}, \qquad k=25\,\text{m}^{-1}, \qquad \omega=7.5\,\text{s}^{-1}.
\] La rapidez de propagación es \[
v=\frac{\omega}{k}=\frac{7.5\,\text{s}^{-1}}{25\,\text{m}^{-1}}=0.30\,\text{m/s}.
\] La amplitud no afecta la rapidez; únicamente determina el
...
\NormalTok{k }\OperatorTok{=} \FloatTok{25.0}        \CommentTok{\# 1/m}
\NormalTok{omega }\OperatorTok{=} \FloatTok{7.5}     \CommentTok{\# 1/s}
\NormalTok{v }\OperatorTok{=}\NormalTok{ omega }\OperatorTok{/}\NormalTok{ k}
\NormalTok{lambda\_ }\OperatorTok{=} \DecValTok{2}\OperatorTok{*}\NormalTok{np.pi }\OperatorTok{/}\NormalTok{ k}
\NormalTok{frequency }\OperatorTok{=}\NormalTok{ omega }\OperatorTok{/}\NormalTok{ (}\DecValTok{2}\OperatorTok{*}\NormalTok{np.pi)}

\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Rapidez de fase: }\SpecialCharTok{\{}\NormalTok{v}\SpecialCharTok{:.3f\}}\SpecialStringTok{ m/s"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Longitud de onda: }\SpecialCharTok{\{}\NormalTok{lambda\_}\SpecialCharTok{:.3f\}}\SpecialStringTok{ m"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Frecuencia: }\SpecialCharTok{\{}\NormalTok{frequency}\SpecialCharTok{:.3f\}}\SpecialStringTok{ Hz"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Rapidez de fase: 0.300 m/s
Longitud de onda: 0.251 m
Frecuencia: 1.194 Hz
\end{verbatim}

\section{Superposición y ondas
estacionarias}\label{superposiciuxf3n-y-ondas-estacionarias}

La ecuación de onda es lineal. Esto significa que si \(y_1(x,t)\) y
\(y_2(x,t)\) son soluciones, entonces cualquier combinación lineal \[
y(x,t)=c_1y_1(x,t)+c_2y_2(x,t)
\] también es solución. Esta propiedad permite explicar la
interferencia, las ondas estacionarias y los modos normales de vibración
(\textbf{french1971?}; \textbf{feynman1964?}).

Una onda estacionaria aparece cuando dos ondas armónicas de la misma
amplitud, frecuencia y número de onda viajan en sentidos opuestos. Por
ejemplo, \[
...
\begin{equation}\phantomsection\label{eq-standing-wave}{
y(x,t)=A_s\sin(kx)\cos(\omega t),
}\end{equation}

\end{tcolorbox}

donde \(A_s\) es la amplitud máxima espacial de la onda estacionaria.
Los puntos donde \(\sin(kx)=0\) son nodos, y los puntos donde
\(|\sin(kx)|=1\) son antinodos.

\subsection{Nodos y antinodos}\label{nodos-y-antinodos}

Los nodos son puntos que permanecen en reposo para todo tiempo. En la
forma \[
y(x,t)=A_s\sin(kx)\cos(\omega t),
...
\section{Ejemplo práctico: análisis de una onda
estacionaria}\label{ejemplo-pruxe1ctico-anuxe1lisis-de-una-onda-estacionaria}

Consideremos el patrón \[
y(x,t)=0.060\sin(4\pi x)\cos(32\pi t),
\] con \(x\) y \(y\) en metros y \(t\) en segundos.

Comparando con la forma estándar \[
y(x,t)=A_s\sin(kx)\cos(\omega t),
\] identificamos \[
A_s=0.060\,\text{m}, \qquad k=4\pi\,\text{m}^{-1}, \qquad \omega=32\pi\,\text{s}^{-1}.
\]

\subsection{Posición de los nodos}\label{posiciuxf3n-de-los-nodos}
...
x_0=0, \qquad x_1=0.25\,\text{m}, \qquad x_2=0.50\,\text{m}.
\] Si se excluye el nodo en el extremo \(x=0\) por considerarlo
frontera, entonces los tres primeros nodos positivos serían
\(0.25\,\text{m}\), \(0.50\,\text{m}\) y \(0.75\,\text{m}\).

\subsection{Período temporal}\label{peruxedodo-temporal}

La frecuencia angular es \(\omega=32\pi\,\text{s}^{-1}\), de modo que \[
T=\frac{2\pi}{\omega}=\frac{2\pi}{32\pi}=0.0625\,\text{s}.
\] Todo punto que no sea nodo oscila con este período.

\subsection{Rapidez de las ondas viajeras
componentes}\label{rapidez-de-las-ondas-viajeras-componentes}

La rapidez de las dos ondas viajeras que interfieren para producir la
onda estacionaria es \[
v=\frac{\omega}{k}=\frac{32\pi}{4\pi}=8.0\,\text{m/s}.
\]

\subsection{Amplitud de las ondas
viajeras}\label{amplitud-de-las-ondas-viajeras}

La onda estacionaria tiene amplitud espacial máxima \(A_s=2A\), donde
\(A\) es la amplitud de cada onda viajera componente. Por tanto, \[
A=\frac{A_s}{2}=\frac{0.060}{2}=0.030\,\text{m}.
\] Cada onda viajera tiene amplitud \(3.0\,\text{cm}\).

\subsection{Instantes de velocidad transversal
nula}\label{instantes-de-velocidad-transversal-nula}

La velocidad transversal es \[
v_y(x,t)=\frac{\partial y}{\partial t}
=-0.060(32\pi)\sin(4\pi x)\sin(32\pi t).
\] Todos los puntos de la cuerda tienen velocidad transversal nula
cuando \[
\sin(32\pi t)=0.
\] Así, \[
32\pi t=m\pi, \qquad m=0,1,2,\dots
\] y \[
t_m=\frac{m}{32}\,\text{s}.
\] Para \(t\geq 0\), los primeros instantes son \[
t_0=0, \qquad t_1=0.03125\,\text{s}, \qquad t_2=0.0625\,\text{s}.
\] Si se busca el primer instante positivo, sería \(0.03125\,\text{s}\).

\begin{Shaded}
\begin{Highlighting}[]
\ImportTok{import}\NormalTok{ numpy }\ImportTok{as}\NormalTok{ np}

\NormalTok{A\_s }\OperatorTok{=} \FloatTok{0.060}
\NormalTok{k }\OperatorTok{=} \DecValTok{4}\OperatorTok{*}\NormalTok{np.pi}
\NormalTok{omega }\OperatorTok{=} \DecValTok{32}\OperatorTok{*}\NormalTok{np.pi}

\NormalTok{nodes }\OperatorTok{=}\NormalTok{ [n}\OperatorTok{*}\NormalTok{np.pi}\OperatorTok{/}\NormalTok{k }\ControlFlowTok{for}\NormalTok{ n }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\DecValTok{4}\NormalTok{)]}
\NormalTok{T }\OperatorTok{=} \DecValTok{2}\OperatorTok{*}\NormalTok{np.pi}\OperatorTok{/}\NormalTok{omega}
\NormalTok{v }\OperatorTok{=}\NormalTok{ omega}\OperatorTok{/}\NormalTok{k}
\NormalTok{A\_traveling }\OperatorTok{=}\NormalTok{ A\_s}\OperatorTok{/}\DecValTok{2}
\NormalTok{t\_zero\_velocity }\OperatorTok{=}\NormalTok{ [m}\OperatorTok{*}\NormalTok{np.pi}\OperatorTok{/}\NormalTok{omega }\ControlFlowTok{for}\NormalTok{ m }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(}\DecValTok{4}\NormalTok{)]}

\BuiltInTok{print}\NormalTok{(}\StringTok{"Nodos (m):"}\NormalTok{, [}\BuiltInTok{round}\NormalTok{(x, }\DecValTok{3}\NormalTok{) }\ControlFlowTok{for}\NormalTok{ x }\KeywordTok{in}\NormalTok{ nodes])}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Periodo: }\SpecialCharTok{\{}\NormalTok{T}\SpecialCharTok{:.3f\}}\SpecialStringTok{ s"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Rapidez: }\SpecialCharTok{\{}\NormalTok{v}\SpecialCharTok{:.3f\}}\SpecialStringTok{ m/s"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Amplitud de cada onda viajera: }\SpecialCharTok{\{}\NormalTok{A\_traveling}\SpecialCharTok{:.3f\}}\SpecialStringTok{ m"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\StringTok{"Instantes con velocidad transversal nula (s):"}\NormalTok{, [}\BuiltInTok{round}\NormalTok{(t, }\DecValTok{3}\NormalTok{) }\ControlFlowTok{for}\NormalTok{ t }\KeywordTok{in}\NormalTok{ t\_zero\_velocity])}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
Nodos (m): [0.0, 0.25, 0.5, 0.75]
Periodo: 0.062 s
Rapidez: 8.000 m/s
Amplitud de cada onda viajera: 0.030 m
Instantes con velocidad transversal nula (s): [0.0, 0.031, 0.062, 0.094]
\end{verbatim}

\section{Visualización con Python}\label{visualizaciuxf3n-con-python}

La visualización ayuda a distinguir una onda viajera de una onda
estacionaria. En una onda viajera, el perfil completo se desplaza. En
una onda estacionaria, los nodos permanecen fijos y cada punto oscila
con una amplitud que depende de su posición.
...
\NormalTok{x }\OperatorTok{=}\NormalTok{ np.linspace(}\DecValTok{0}\NormalTok{, }\FloatTok{1.0}\NormalTok{, }\DecValTok{600}\NormalTok{)}
\NormalTok{times }\OperatorTok{=}\NormalTok{ [}\FloatTok{0.0}\NormalTok{, }\FloatTok{0.0078125}\NormalTok{, }\FloatTok{0.015625}\NormalTok{, }\FloatTok{0.0234375}\NormalTok{, }\FloatTok{0.03125}\NormalTok{]}

\NormalTok{plt.figure(figsize}\OperatorTok{=}\NormalTok{(}\DecValTok{8}\NormalTok{, }\FloatTok{4.5}\NormalTok{))}
\ControlFlowTok{for}\NormalTok{ t }\KeywordTok{in}\NormalTok{ times:}
\NormalTok{    y }\OperatorTok{=}\NormalTok{ A\_s}\OperatorTok{*}\NormalTok{np.sin(k}\OperatorTok{*}\NormalTok{x)}\OperatorTok{*}\NormalTok{np.cos(omega}\OperatorTok{*}\NormalTok{t)}
\NormalTok{    plt.plot(x, y, label}\OperatorTok{=}\SpecialStringTok{f"t = }\SpecialCharTok{\{}\NormalTok{t}\SpecialCharTok{:.5f\}}\SpecialStringTok{ s"}\NormalTok{)}

\NormalTok{plt.axhline(}\DecValTok{0}\NormalTok{, linewidth}\OperatorTok{=}\FloatTok{0.8}\NormalTok{)}
\NormalTok{plt.xlabel(}\StringTok{"x (m)"}\NormalTok{)}
\NormalTok{plt.ylabel(}\StringTok{"y(x,t) (m)"}\NormalTok{)}
\NormalTok{plt.title(}\StringTok{"Evolución temporal de una onda estacionaria"}\NormalTok{)}
...
\pandocbounded{\includegraphics[keepaspectratio]{chapters/ondas_ecuacion_y_superposicion_files/figure-pdf/cell-4-output-1.pdf}}

Para localizar nodos de forma computacional, basta usar la fórmula
analítica \(x_n=n\pi/k\). En un análisis numérico con datos discretos,
también puede estimarse la posición de nodos buscando puntos donde la
amplitud temporal sea cercana a cero.
...
\NormalTok{n\_max }\OperatorTok{=} \BuiltInTok{int}\NormalTok{(np.floor(k}\OperatorTok{*}\NormalTok{L}\OperatorTok{/}\NormalTok{np.pi))}
\NormalTok{nodes }\OperatorTok{=}\NormalTok{ np.array([n}\OperatorTok{*}\NormalTok{np.pi}\OperatorTok{/}\NormalTok{k }\ControlFlowTok{for}\NormalTok{ n }\KeywordTok{in} \BuiltInTok{range}\NormalTok{(n\_max }\OperatorTok{+} \DecValTok{1}\NormalTok{)])}

\BuiltInTok{print}\NormalTok{(nodes)}
\end{Highlighting}
\end{Shaded}
...
\section{Ejemplo práctico: distancia recorrida por las ondas
componentes}\label{ejemplo-pruxe1ctico-distancia-recorrida-por-las-ondas-componentes}

Ahora consideremos dos ondas que viajan en sentidos opuestos: \[
y_1(x,t)=(5.00\,\text{mm})\sin(6.00\pi x-300\pi t),
\] \[
y_2(x,t)=(5.00\,\text{mm})\sin(6.00\pi x+300\pi t).
\] Su superposición es \[
y(x,t)=y_1+y_2=2(5.00\,\text{mm})\sin(6.00\pi x)\cos(300\pi t).
\] Por tanto, el patrón estacionario tiene amplitud máxima \[
A_s=10.00\,\text{mm}.
\]

Aquí \[
k=6\pi\,\text{m}^{-1}, \qquad \omega=300\pi\,\text{s}^{-1}.
\] La rapidez de cada onda viajera es \[
v=\frac{\omega}{k}=\frac{300\pi}{6\pi}=50\,\text{m/s}.
\]

Un antinodo pasa de desplazamiento máximo hacia arriba a desplazamiento
máximo hacia abajo en medio período. El período de oscilación es \[
T=\frac{2\pi}{\omega}=\frac{2\pi}{300\pi}=0.00667\,\text{s}.
\] Entonces el intervalo requerido es \[
\Delta t=\frac{T}{2}=0.00333\,\text{s}.
\] Durante ese tiempo, cada onda viajera recorre una distancia \[
\Delta x=v\Delta t=(50\,\text{m/s})(0.00333\,\text{s})\approx 0.167\,\text{m}.
\]
...
\Delta x=\frac{\lambda}{2}=\frac{1}{6}\,\text{m}\approx 0.167\,\text{m}.
\]
...
\NormalTok{A }\OperatorTok{=} \FloatTok{5.00e{-}3}
\NormalTok{k }\OperatorTok{=} \DecValTok{6}\OperatorTok{*}\NormalTok{np.pi}
\NormalTok{omega }\OperatorTok{=} \DecValTok{300}\OperatorTok{*}\NormalTok{np.pi}

\NormalTok{v }\OperatorTok{=}\NormalTok{ omega}\OperatorTok{/}\NormalTok{k}
\NormalTok{T }\OperatorTok{=} \DecValTok{2}\OperatorTok{*}\NormalTok{np.pi}\OperatorTok{/}\NormalTok{omega}
\NormalTok{dt }\OperatorTok{=}\NormalTok{ T}\OperatorTok{/}\DecValTok{2}
\NormalTok{distance }\OperatorTok{=}\NormalTok{ v}\OperatorTok{*}\NormalTok{dt}
\NormalTok{lambda\_ }\OperatorTok{=} \DecValTok{2}\OperatorTok{*}\NormalTok{np.pi}\OperatorTok{/}\NormalTok{k}

\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Rapidez: }\SpecialCharTok{\{}\NormalTok{v}\SpecialCharTok{:.1f\}}\SpecialStringTok{ m/s"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Periodo: }\SpecialCharTok{\{}\NormalTok{T}\SpecialCharTok{:.4f\}}\SpecialStringTok{ s"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Intervalo de máximo superior a máximo inferior: }\SpecialCharTok{\{}\NormalTok{dt}\SpecialCharTok{:.4f\}}\SpecialStringTok{ s"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Distancia recorrida: }\SpecialCharTok{\{}\NormalTok{distance}\SpecialCharTok{:.3f\}}\SpecialStringTok{ m"}\NormalTok{)}
\BuiltInTok{print}\NormalTok{(}\SpecialStringTok{f"Longitud de onda: }\SpecialCharTok{\{}\NormalTok{lambda\_}\SpecialCharTok{:.3f\}}\SpecialStringTok{ m"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Rapidez: 50.0 m/s
Periodo: 0.0067 s
Intervalo de máximo superior a máximo inferior: 0.0033 s
Distancia recorrida: 0.167 m
Longitud de onda: 0.333 m
\end{verbatim}

\section{Interpretación física de los nodos y la
energía}\label{interpretaciuxf3n-fuxedsica-de-los-nodos-y-la-energuxeda}

Aunque una onda estacionaria parece no propagarse globalmente, puede
entenderse como la interferencia de dos ondas viajeras que transportan
energía en direcciones opuestas. En un patrón ideal con amplitudes
iguales, el flujo neto promedio de energía puede anularse, pero
localmente la energía se transforma entre energía cinética y energía
potencial elástica.

En los nodos, el desplazamiento transversal es siempre cero; sin
embargo, la cuerda no necesariamente carece de energía cerca de ellos,
pues la pendiente espacial puede ser máxima. En los antinodos ocurre lo
contrario: la amplitud de desplazamiento es máxima, y el punto alterna
entre máxima elongación y paso por el equilibrio con máxima rapidez
transversal.

Esta distinción es importante en sistemas físicos más generales. Los
modos normales de una cuerda, una columna de aire o una cavidad
electromagnética se describen mediante patrones estacionarios
determinados por condiciones de frontera. La cuantización de modos en
sistemas vibrantes y en campos físicos surge precisamente de imponer que
sólo ciertas longitudes de onda sean compatibles con la geometría del
sistema (\textbf{feynman1964?}; \textbf{kleppner2014?}).

\section{Resumen de fórmulas
útiles}\label{resumen-de-fuxf3rmulas-uxfatiles}
...
\] Si la onda estacionaria resulta de la superposición de dos ondas
viajeras idénticas de amplitud \(A\), entonces \[
A_s=2A.
\]
...
describir perturbaciones que se propagan en medios físicos. A partir de
ella se reconocen soluciones viajeras, ondas radiales esféricas y
patrones estacionarios. La forma funcional de la onda permite
identificar de manera directa la rapidez de propagación, la frecuencia,
la longitud de onda y el período.

Las ondas estacionarias muestran cómo la superposición de ondas viajeras
puede producir estructuras espaciales fijas. Los nodos y antinodos no
son propiedades de una sola onda viajera, sino del patrón de
interferencia. Esta idea es esencial para entender modos normales,
resonancia, condiciones de frontera y fenómenos ondulatorios en sistemas
mecánicos, acústicos y electromagnéticos.

El tratamiento computacional en Python complementa el análisis
analítico: permite visualizar la evolución temporal, verificar fórmulas
y explorar cómo cambian los patrones al modificar \(k\), \(\omega\) o la
amplitud. En un curso técnico, esta combinación entre deducción
matemática, interpretación física y experimentación numérica favorece
una comprensión más profunda del fenómeno ondulatorio.
